...
const lastWorldTileWidth = 120; // Width of each tile in MRMO_BRIK.png
const lastWorldTileHeight = 160; // Height of each tile in MRMO_BRIK.png
let isPico8World = false; // Flag to track if we're in the new world
...
function p3_drawTile(i, j) {
  ...
  const overlayHash = XXH.h32("overlay:" + [i, j], worldSeed) % 500;
  if (overlayHash === 0 && portalSpritesheet) {
  ...
  // Draw tiles for the first world
  let baseTileType;
...
function determineLastWorldTileType(i, j) {
  // Placeholder logic for the last world tiles
  return [0, 0]; // Modify this logic as needed
}
